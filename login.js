...
function accountEmails(value){return [`admin-${value}@haleh-crm.local`,`staff-${value}@haleh-crm.local`]}
async function signIn(){
 const normalized=phone.value.replace(/[۰-۹]/g,d=>'۰۱۲۳۴۵۶۷۸۹'.indexOf(d)).replace(/[٠-٩]/g,d=>'٠١٢٣٤٥٦٧٨٩'.indexOf(d));
 let response,result;
 for(const email of accountEmails(normalized)){
  response=await fetch(`${SUPABASE_URL}/auth/v1/token?grant_type=password`,{method:'POST',headers:{apikey:SUPABASE_KEY,'Content-Type':'application/json'},body:JSON.stringify({email,password:password.value})});
  result=await response.json();if(response.ok&&result.access_token)break;
 }
